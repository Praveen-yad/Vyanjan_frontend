...
function Card({data}){
    ...
    const AddToCart = async(id) => {
        const json = await axios.put('http://localhost:5000/api/upcart',{
            email: email,
            id: id,
            amount:count,
            size:size,
    ...
    const RemoveHandler = async(id) => {
        const json = await axios.put('http://localhost:5000/api/decart',{
            email: email,
            id: id
        })
        // console.log(json)
  ...
  return (
    <div className=' bg-neutral-900 h-[16rem] rounded-2xl overflow-hidden min-w-[28rem] shadow-[1px_1px_10px_0px] shadow-black flex items-center '>
        ...
        <div className='mx-3 my-1 w-[13rem]'>
            ...
            <div className='text-[13px] select-none'>{data.description.slice(0,120)}</div>
            <div className='flex items-center justify-between mx-[1px] mt-5'>
                ...
                <div>
                    <select onChange={(e) => setSize(e.target.value)} className='bg-theme outline-none rounded-[4px] px-1 select-none'>
            ...
            <div className='flex mt-4 justify-between'>
                ...
                {toggle ? (token) ? 
                    <motion.div whileTap={{scale:0.97}} className='bg-theme px-2 py-1 rounded-md text-center hover:outline outline-[1.5px] cursor-pointer'onClick={() => AddToCart(data._id)} >Add to cart</motion.div> 
                :
                    <div className=' bg-theme px-2 py-1 rounded-md text-center hover:outline hover:opacity-75 outline-[1.5px] cursor-not-allowed'>Add to Cart</div>
                :
