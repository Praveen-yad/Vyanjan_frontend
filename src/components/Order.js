...
const Order = ({setToggle, cartArray, setPlaced}) => {
    ...
    const PlaceOrder = async() => {
        ...
        await axios.post('http://localhost:5000/api/emptycart',{
            email:localStorage.getItem('email')
        })
    }

  return (
    <div className='bg-white rounded-2xl w-[33rem] h-[25rem] py-2 px-2'>
        <div>
            ...
            <div className='px-5 mt-5'>
                ...
                <div className='flex justify-between mt-2'>
                    <div className='w-[9rem] h-[5rem] rounded-lg p-2 bg-neutral-900 flex items-center justify-center cursor-not-allowed hover:bg-opacity-95'>
                    ...
                    <motion.div onClick={() => setcash(!cash)} whileTap={{scale:0.97}} className={`w-[9rem] h-[5rem] rounded-lg p-2 bg-neutral-900 flex items-center justify-center ${cash ? 'outline outline-theme outline-4': ''}`}>
                ...
                <div className='mt-5 '>
                    ...
                    <div className='flex justify-between text-white mt-2'>
                        {[0, 30, 60].map((item, index) => (
                            <motion.div onClick={() => setTip(item)} className={`w-24 h-20 bg-neutral-900 flex justify-center items-center rounded-lg ${tip === item && 'outline outline-theme outline-4'}`} whileTap={{scale:0.97}} key={index}><FcMoneyTransfer/>{item}</motion.div>
                        ))}
                        <motion.div className='w-24 h-20 bg-theme flex justify-center items-center rounded-lg cursor-pointer' whileTap={{scale:0.97}} onClick={PlaceOrder}><CiSaveUp1 className='text-3xl rotate-90'/></motion.div>
                    </div>
                </div>
            </div>
        </div>
